...
data class DeclineRequest(
    @property:LLMDescription("Calendar event id Baruch is declining (from calendar-mcp)")
    val eventId: String,
    ...
    @property:LLMDescription("Names of people attending who can corroborate or contradict a story")
...
@LLMDescription("A decline j-claw has drafted, staged, and is ready to send")
@Serializable
data class DeclineDeployment(
    @property:LLMDescription("Excuse flavor selected")
    val flavor: ExcuseFlavor,

    @property:LLMDescription("Calendar event id created to back the story (from createCalendarEvent)")
    ...
    @property:LLMDescription("Hallway script — what to say if the organizer asks about it the next day")
